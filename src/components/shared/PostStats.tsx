...
const PostStats = ({post,userId} : PostStatsProps) => {
 ...
 //   const savedPostRecord = currentUser?.save.find((record:Models.Document) => record.post.$id === post.$id);
    ...
    const savedPostRecord = currentUser?.save.find(
      ...
      if (savedPostRecord) {
       // console.log(savedPostRecord.record.post.$id);
      //  console.log(savedPostRecord.post.$id);
      }
   ...
   /* const handleSavePost = (e: React.MouseEvent) => {
        ...
        if(savedPostRecord){
            console.log(savedPostRecord);
            setIsSaved(false);
            deleteSavePost(savedPostRecord);
        }else{
    ...
    const handleSavePost = (e: React.MouseEvent) => {
        ...
        if (savedPostRecord) {
            setIsSaved(false);
            deleteSavePost(savedPostRecord.$id); // Pass the documentId instead of the entire object
        } else {
            savePost({ postId: post?.$id || '', userId });
            setIsSaved(true);
        }
